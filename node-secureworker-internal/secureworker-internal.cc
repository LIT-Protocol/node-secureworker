...
{
  {
    ...
    if (status != SGX_SUCCESS)
      throw sgx_error(status, "sgx_create_enclave");
  }
...
void SecureWorkerInternal::destroy()
{
  ...
  if (status != SGX_SUCCESS)
    throw sgx_error(status, "sgx_destroy_enclave");
  enclave_id = 0;
}
...
{
  {
    ...
    if (status != SGX_SUCCESS)
      throw sgx_error(status, "duk_enclave_init");
  }
}
...
{
  {
    ...
    if (status != SGX_SUCCESS)
      throw sgx_error(status, "duk_enclave_close");
  }
}
...
{
  {
    ...
    if (status != SGX_SUCCESS)
      throw sgx_error(status, "duk_enclave_emit_message");
  }
}
...
{
  {
    ...
    if (status != SGX_SUCCESS)
      throw sgx_error(status, "duk_enclave_bootstrap_mock");
  }
}
...
{
  {
    ...
    printf("running sgx_init_quote\n");
    const sgx_status_t status = sgx_init_quote(target_info, gid);
    if (status != SGX_SUCCESS)
      throw sgx_error(status, "sgx_init_quote");
  }
}
...
{
  {
    ...
    if (status != SGX_SUCCESS)
      throw sgx_error(status, "sgx_get_quote_size");
  }
}
...
{
  {
    ...
    if (rc != 1)
      throw sgx_error(SGX_ERROR_UNEXPECTED, "sgx_get_quote nonce generation");
    ...
    if (status != SGX_SUCCESS)
      throw sgx_error(status, "sgx_get_quote");

    SHA256_CTX context;
    if (!SHA256_Init(&context))
      throw sgx_error(SGX_ERROR_UNEXPECTED, "SHA256_Init");

    if (!SHA256_Update(&context, &nonce, sizeof(nonce)))
      throw sgx_error(SGX_ERROR_UNEXPECTED, "SHA256_Update nonce");

    if (!SHA256_Update(&context, quote, quote_size))
      throw sgx_error(SGX_ERROR_UNEXPECTED, "SHA256_Update quote");

    unsigned char digest[sizeof(sgx_report_data_t)] = {0};

    if (!SHA256_Final(digest, &context))
      throw sgx_error(SGX_ERROR_UNEXPECTED, "SHA256_Final");

    // Report data from qe_report should be equal to sha256(nonce || quote).
    if (memcmp(&qe_report.body.report_data, digest, sizeof(sgx_report_data_t)) != 0)
      throw sgx_error(SGX_ERROR_UNEXPECTED, "sgx_get_quote nonce mismatch");
    ;
  }
}
...
void duk_enclave_post_message(const char *message)
{
  ...
  if (!handle_post_message->IsFunction())
    return;
